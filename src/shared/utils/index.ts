...
import { clearAuthSession, getStoredAuthToken, getStoredAuthSession, persistAuthSession } from './authSession';
...
export const auth = {
  ...
  setToken: (token: string): void => {
    ...
    if (user) persistAuthSession(token, user);
  },
  ...
  getUser: () => {
    return getStoredAuthSession().user;
  },
...
export const api = {
  ...
  handleResponse: async <T>(response: Response): Promise<T> => {
    if (!response.ok) {
      const error = await response.json().catch((): { message?: string } => ({ message: 'Error del servidor' }));
      throw new Error(typeof error.message === 'string' ? error.message : 'Error del servidor');
    }
